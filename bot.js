...
client.on('ready', () => {
    ...
    console.log(`Channels : [ " ${client.channels.size} " ]`);
     client.user.setActivity("THE FALL.",{type: 'WATCHING'})
...
${prefix}mute @user <reason> / اعطاء العضو ميوت 
...
${prefix}color <number> / لإختيار لون
${prefix}tm <user> <time> <reason> / ميوت بوقت
${prefix}gstart / جيف أواي
**
   `/* ,`
        ***__Music orders__***
**
...
client.on("message", message => {
    ...
    var rebel = `
      **=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=**
             ...
             ** ${prefix}banlist **    |    قائمة المبندين

             ** ${prefix}slots **    |    لعبة الحظ

             ** ${prefix}user **    |    معلومات الشخص بطريقة أخرى
   ...
   client.on("message", message => {
    ...
    var rebel = `
             ...
             ** ${prefix}autoc **    |    يعطي رول بالرياكشن

             ** ${prefix}temp on **    |    تشغيل خاصية الروم المأقت

             ** ${prefix}temp off **    |    تعطيل خاصية الروم المأقت

             ** ${prefix}temp time **    |    تحديد الوقت للروم المأقت

             ** ${prefix}voicesetup **    |    تشغيل خاصية الفويس اونلاين

             ** ${prefix}fastrandom **    |    يسمح لأونر السيرفر بإختيار عضو عشوائي
...
/*
             ...
             **=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=**
                        ...
                        **Inive Link        :     https://goo.gl/dah9qK ** 

                        **The Fall Support        :     https://discord.gg/uYHwzaX ** 
...
/*
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/uYHwzaX**")
...
client.on('guildCreate', guild => {
     ...
     .setURL('https://discordapp.com/oauth2/authorize?client_id=543708898964668417&scope=bot&permissions=8')
  .setDescription(`**
  New Server Add The Fall © ?
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("543709135032811530").sendEmbed(embed)
});
...
if (message.author.id !== '542787046142640138') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
...
client.on('message', message => {
if(message.content.includes("<@543708898964668417>")) {
